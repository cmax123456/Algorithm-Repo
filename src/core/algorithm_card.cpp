...
json ToJson(const AlgorithmCard& card) {
    ...
    if (card.resource_requirements.has_value()) {
        ...
        WriteOptional(resource_json, "bandwidth_requirement_mbps",
                      card.resource_requirements->bandwidth_requirement_mbps);
    ...
    if (card.deployment.has_value()) {
        json deployment_json;
        json locations_json = json::array();
        for (const auto& location : card.deployment->locations) {
        ...
        deployment_json["locations"] = std::move(locations_json);
        deployment_json["placement_policy"] = card.deployment->placement_policy;
        WriteOptional(deployment_json, "replica_count", card.deployment->replica_count);
        card_json["deployment"] = std::move(deployment_json);
    }
    ...
    try {
        ...
        if (json_value.contains("resource_requirements")) {
            ...
            resource_requirements.bandwidth_requirement_mbps =
                ReadOptional<int>(resource_json, "bandwidth_requirement_mbps");
        ...
        if (json_value.contains("deployment")) {
            const json& deployment_json = json_value.at("deployment");
            if (!deployment_json.is_object()) {
                return Status::Error(ErrorCode::kInvalidAlgorithmCard,
                                     "deployment must be an object.");
            }
            DeploymentSpec deployment;
            if (deployment_json.contains("locations")) {
                if (!deployment_json.at("locations").is_array()) {
                    return Status::Error(ErrorCode::kInvalidAlgorithmCard,
                                         "deployment.locations must be an array.");
                }
                for (const auto& location_json : deployment_json.at("locations")) {
                    DeploymentLocation location;
                    location.node_id = location_json.value("node_id", std::string());
                    location.node_address = location_json.value("node_address", std::string());
                    location.region = location_json.value("region", std::string());
                    deployment.locations.push_back(std::move(location));
                }
            }
            deployment.placement_policy =
                deployment_json.value("placement_policy", std::string());
            deployment.replica_count = ReadOptional<int>(deployment_json, "replica_count");
            card.deployment = std::move(deployment);
        }
