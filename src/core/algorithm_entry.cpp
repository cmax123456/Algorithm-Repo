...
nlohmann::json ToAgentViewJson(const AlgorithmEntry& entry) {
    ...
    for (const auto& example : entry.card.agent_card.examples) {
        agent_view["agent_card"]["examples"].push_back({
            {"input", example.input},
    ...
    agent_view["performance"] =
        entry.card.performance.has_value() ? ToJson(entry.card).value("performance",
                                                                      nlohmann::json::object())
                                           : nlohmann::json::object();
    agent_view["hardware_requirements"] =
        entry.card.hardware_requirements.has_value()
            ? ToJson(entry.card).value("hardware_requirements", nlohmann::json::object())
            : nlohmann::json::object();
    agent_view["safety"] =
