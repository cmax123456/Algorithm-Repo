...
std::vector<std::string> ExtractTypeNames(const json& schema_json) {
    std::vector<std::string> type_names;
    if (!schema_json.contains("type")) {
        return type_names;
    }
    const json& type_node = schema_json.at("type");
    if (type_node.is_string()) {
        type_names.push_back(type_node.get<std::string>());
    } else if (type_node.is_array()) {
        for (const auto& item : type_node) {
            if (item.is_string()) {
                type_names.push_back(item.get<std::string>());
            }
        }
    }
    return type_names;
...
bool SchemaTypeIncludes(const json& schema_json, const std::string& type_name) {
    const std::vector<std::string> type_names = ExtractTypeNames(schema_json);
...
Status ValidateSchemaNode(const json& schema_json,
    ...
    if (schema_json.contains("type")) {
        const json& type_node = schema_json.at("type");
        if (!type_node.is_string() && !type_node.is_array()) {
            return MakeSchemaError(
                invalid_schema_code, schema_name,
                "type must be a string or an array of strings in the supported subset.");
        }
        if (type_node.is_array() && type_node.empty()) {
            return MakeSchemaError(invalid_schema_code, schema_name,
        ...
        if (type_node.is_array()) {
            for (const auto& item : type_node) {
                if (!item.is_string()) {
                    return MakeSchemaError(
                        invalid_schema_code, schema_name,
                        "type array items must be strings in the supported subset.");
                }
            }
        }
        for (const auto& type_name : ExtractTypeNames(schema_json)) {
...
bool MatchesTypeName(const json& instance_json, const std::string& type_name) {
    return (type_name == "object" && instance_json.is_object()) ||
...
Status ValidateTypeConstraint(const json& instance_json,
    ...
    for (const auto& type_name : type_names) {
        if (MatchesTypeName(instance_json, type_name)) {
            type_matches = true;
            break;
        }
    }
    if (!type_matches) {
        ...
        for (std::size_t index = 0; index < type_names.size(); ++index) {
            ...
            joined_type_names += type_names[index];
        }
        return MakeSchemaError(invalid_schema_code, instance_name,
                               "Value does not match schema type " + joined_type_names + ".");
    }
    return Status::Ok();
}
...
Status ValidateInstanceNode(const json& instance_json,
    ...
    if (schema_json.contains("type")) {
        const auto type_status = ValidateTypeConstraint(
            instance_json, schema_json, invalid_schema_code, instance_name);
        if (!type_status.ok()) {
            return type_status;
        }
    }

    if ((SchemaTypeIncludes(schema_json, "object") || SchemaHasObjectKeywords(schema_json)) &&
    ...
    if ((SchemaTypeIncludes(schema_json, "array") || SchemaHasArrayKeywords(schema_json)) &&
    ...
    if ((SchemaTypeIncludes(schema_json, "string") || SchemaHasStringKeywords(schema_json)) &&
    ...
    if ((SchemaTypeIncludes(schema_json, "number") ||
         SchemaTypeIncludes(schema_json, "integer") ||
