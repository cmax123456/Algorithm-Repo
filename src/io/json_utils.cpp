...
// zh: schema type keyword may be a single string or a union-type array (e.g.
// ["integer", "null"]) per standard JSON Schema. Preserve the original shape
// instead of forcing a string conversion, which would throw for arrays.
nlohmann::json ExtractTypeSummary(const nlohmann::json& schema_node) {
    if (!schema_node.contains("type")) {
        return "";
    }
    const nlohmann::json& type_node = schema_node.at("type");
...
nlohmann::json JsonUtils::SummarizeSchema(const nlohmann::json& schema_json) {
    ...
    summary["type"] = ExtractTypeSummary(schema_json);
    ...
    if (schema_json.contains("properties") && schema_json.at("properties").is_object()) {
        for (auto it = schema_json.at("properties").begin();
            ...
            summary["properties"].push_back({
                {"name", it.key()},
                {"type", ExtractTypeSummary(property_schema)},
