...
AlgorithmResult ExecutionCoordinator::Run(const AlgorithmRequest& request) {
    ...
    // entry 是当前请求取得的值快照；在途请求不应因并发 /reload 改变其校验契约。
    auto input_status = registry_.ValidateInputPayload(entry, effective_request.inputs);
    if (!input_status.ok()) {
    ...
    auto output_status = registry_.ValidateOutputPayload(entry, result.outputs);
    if (!output_status.ok()) {
