...
std::unordered_map<std::string, std::weak_ptr<std::mutex>> g_states;

std::shared_ptr<std::mutex> SharedStateForPath(const std::filesystem::path& log_path) {
    const std::string key = std::filesystem::absolute(log_path).generic_string();
    std::lock_guard<std::mutex> lock(g_states_mutex);
    const auto it = g_states.find(key);
    if (it != g_states.end()) {
...
Status ExecutionLogger::Append(const AlgorithmRequest& request,
    ...
    if (result.function_execution.has_value()) {
        log_record["function_execution"] = ToJson(result).at("function_execution");
    }
