...
Result<AlgorithmQuery> ParseAlgorithmQuery(const httplib::Request& request) {
    AlgorithmQuery query;
    if (request.has_param("algorithm_id")) {
        query.algorithm_id = request.get_param_value("algorithm_id");
    }
    if (request.has_param("task_family")) {
        query.task_family = request.get_param_value("task_family");
    }
    if (request.has_param("capability")) {
        query.capability = request.get_param_value("capability");
    }
    if (request.has_param("status")) {
        auto status_result = ParseAlgorithmStatus(request.get_param_value("status"));
        if (!status_result.ok()) {
            return status_result.status();
        }
        query.status = status_result.value();
    }
    if (request.has_param("max_memory_mb")) {
        try {
            query.max_memory_footprint_mb = std::stoi(request.get_param_value("max_memory_mb"));
        } catch (const std::exception&) {
            return InvalidArgument("max_memory_mb must be an integer.");
        }
    }
    if (request.has_param("max_bandwidth_mbps")) {
        try {
            query.max_bandwidth_requirement_mbps =
                std::stoi(request.get_param_value("max_bandwidth_mbps"));
        } catch (const std::exception&) {
            return InvalidArgument("max_bandwidth_mbps must be an integer.");
        }
    }
    if (request.has_param("preferred_compute")) {
        query.preferred_compute = request.get_param_value("preferred_compute");
    }
    if (request.has_param("runtime_state")) {
        auto state_result = ParseRuntimeState(request.get_param_value("runtime_state"));
        if (!state_result.ok()) {
    ...
    void RegisterRoutes() {
        ...
        server_.Get("/algorithms/query",
                    [this](const httplib::Request& request, httplib::Response& response) {
                        ...
                        auto query_result = ParseAlgorithmQuery(request);
                        if (!query_result.ok()) {
                            WriteJson(&response, HttpStatusForStatus(query_result.status()),
                                      ErrorPayload(query_result.status()));
                            return;
                        }

                        json results_json = json::array();
                        for (const auto& item : registry_.Query(query_result.value())) {
                            results_json.push_back(ToQueryResultJson(item));
                        }
                        WriteJson(&response, 200,
                                  json{{"ok", true},
        ...
        server_.Get(R"(/algorithms/([^/]+)/([^/]+)/([^/]+)/model-info)",
                    [this](const httplib::Request& request, httplib::Response& response) {
                        auto key_result = ParseKeyFromMatches(request);
                        if (!key_result.ok()) {
                            WriteJson(&response, HttpStatusForStatus(key_result.status()),
                        ...
                        json payload = ToModelInfoJson(entry_result.value());
                        payload["ok"] = true;
                        WriteJson(&response, 200, payload);
                    });
