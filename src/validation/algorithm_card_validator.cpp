...
Result<AlgorithmCard> ParseAlgorithmCard(const YAML::Node& root) {
    ...
    if (root["resource_requirements"] && root["resource_requirements"].IsMap()) {
        ...
        if (root["resource_requirements"]["bandwidth_requirement_mbps"]) {
            resource_requirements.bandwidth_requirement_mbps =
                root["resource_requirements"]["bandwidth_requirement_mbps"].as<int>();
        }
    ...
    if (root["deployment"] && root["deployment"].IsMap()) {
        DeploymentSpec deployment;
        if (root["deployment"]["locations"] && root["deployment"]["locations"].IsSequence()) {
            for (const auto& location_node : root["deployment"]["locations"]) {
                DeploymentLocation location;
                if (location_node["node_id"]) {
                    location.node_id = location_node["node_id"].as<std::string>();
                }
                if (location_node["node_address"]) {
                    location.node_address = location_node["node_address"].as<std::string>();
                }
                if (location_node["region"]) {
                    location.region = location_node["region"].as<std::string>();
                }
                deployment.locations.push_back(std::move(location));
            }
        }
        if (root["deployment"]["placement_policy"]) {
            deployment.placement_policy =
                root["deployment"]["placement_policy"].as<std::string>();
        }
        if (root["deployment"]["replica_count"]) {
            deployment.replica_count = root["deployment"]["replica_count"].as<int>();
        }
